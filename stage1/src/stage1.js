...
export async function runScrape({
  ...
  keepOpen = false,
  saveJsonFile = true,
  // Pipeline control: when true, collect interactions (existing behavior).
  // When false, do NOT open reactions modal; instead capture author + post content for each post.
  scrapeLikes = true,
}) {
  ...
  try {
    ...
    if (searchUrl || keywords) {
      ...
      console.log(`[${nowTs()}] Processing ${postCount} posts (scrapeLikes=${!!scrapeLikes}).`);
      ...
      for (let idx = 0; idx < posts.length; idx++) {
        ...
        try {
          if (scrapeLikes) {
            // Existing behavior: open reactions and collect reactors (no like count persistence requested)
            ...
            // New behavior: do NOT collect interactions. Store only post author + content using given XPaths.
            const authorLink = await postHandle.$("xpath=.//div[contains(@class, 'update-components-actor__meta')]//a[contains(@class,'update-components-actor__meta-link')]");
            if (!authorLink) { continue; }
            const href = await authorLink.getAttribute('href');
            if (!href) { continue; }
            const fullUrl = href.startsWith('http') ? href : `https://www.linkedin.com${href}`;
            const match = fullUrl.match(/linkedin\.com\/in\/([^\/?#]+)/i);
            if (!match) { continue; } // skip non-/in/ authors per requirement
            const lead_id = match[1];

            // authorTitle -> bio
            let authorTitle = '';
            try {
              const titleEl = await postHandle.$("xpath=.//div[contains(@class, 'update-components-actor__meta')]//span[contains(@class,'update-components-actor__description')]");
              if (titleEl) {
                authorTitle = (await titleEl.evaluate(el => (el.innerText || el.textContent || '').trim())) || '';
              }
            } catch {}
            ...
            let postContent = '';
            try {
              const contentEl = await postHandle.$("xpath=.//div[contains(@class, 'update-components-text') and contains(@class, 'update-components-update-v2__commentary')]");
              if (contentEl) {
                postContent = (await contentEl.evaluate(el => (el.innerText || el.textContent || '').trim())) || '';
              }
            } catch {}

            if (!allLeadsMap.has(lead_id)) {
              allLeadsMap.set(lead_id, { lead_id, linkedin_url: fullUrl, bio: authorTitle || null, post_content: postContent || null });
            } else {
              // Overwrite behavior as accepted: update bio/post_content for existing lead_id
              const prev = allLeadsMap.get(lead_id);
              allLeadsMap.set(lead_id, { ...prev, bio: authorTitle || prev.bio || null, post_content: postContent || prev.post_content || null });
            }
          }
        } catch (e) {
