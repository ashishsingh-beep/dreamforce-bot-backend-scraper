...
function validatePayload(body) {
  ...
  const { emailMode, email, password, accountStatus, accountEmail, keyword, searchUrl, durationSec, userId, tag, scrapeLikes } = body;
  ...
  if (scrapeLikes !== undefined && typeof scrapeLikes !== 'boolean') errors.push('scrapeLikes must be a boolean');
  if (emailMode === 'manual') {
...
app.post('/scrape', async (req, res) => {
  ...
  try {
    result = await runScrape({
      ...
      // Pipeline control: default true (collect interactions)
      scrapeLikes: typeof req.body.scrapeLikes === 'boolean' ? req.body.scrapeLikes : true,
      keepOpen: false,
      saveJsonFile: true,
    });
  } catch (e) {
        ...
        for (let attempt = 0; attempt < accounts.length; attempt++) {
          ...
          try {
            result = await runScrape({
              ...
              headless: false,
              // Respect new per-request flag (default true when undefined)
              scrapeLikes: typeof requestRow.scrape_likes === 'boolean' ? requestRow.scrape_likes : true,
              keepOpen: false,
              saveJsonFile: true
            });
            try {
...
// Only start server if this file is the entrypoint (robust on Windows)
try {
