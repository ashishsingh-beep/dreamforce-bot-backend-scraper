...
export async function saveAllLeads(leads, userId = null, tag = null) {
  ...
  const payload = leads.map(l => {
    ...
    // Include optional post_content when provided (column exists per request)
    if (typeof l.post_content === 'string' && l.post_content.length >= 0) {
      row.post_content = l.post_content;
    }
    return row;
  });
